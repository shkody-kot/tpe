...
uint8_t * tpe::decrypt(uint8_t * image, uint8_t * sub_array, uint8_t * perm_array, int width, int height)
{
	...
	std::thread thread1(decrypt_thread, one, blocksize, iterations, total_for_sub, total_for_perm);
	std::thread thread2(decrypt_thread, two, blocksize, iterations, total_for_sub, total_for_perm);
	std::thread thread3(decrypt_thread, three, blocksize, iterations, last_block_sub, last_block_perm);
...
void tpe::decrypt_thread(pixels * data, int blocksize, int iterations, int total_for_sub, int total_for_perm)
{
	std::vector<int> permutation;
	uint8_t r1, b1, g1, r2, b2, g2, rt1, gt1, bt1, rt2, gt2, bt2;
	int p, q, x, y;
	...
	for (int ccc = 0; ccc < iterations; ccc++)
	{
		data->sub->set_ctr((iterations - (ccc + 1)) * (total_for_sub / iterations));
		data->perm->set_ctr((iterations - (ccc + 1)) * (total_for_perm / iterations));
		...
		{			
			for (int j = 0; j < data->block_width; j++)
			{
				...
				for (int k = 0; k < blocksize * blocksize; k++)
				{
					p = permutation[k] / blocksize;
					q = permutation[k] % blocksize;
					data->image[(i * data->width * blocksize + p * data->width + j * blocksize + q) * 4] = r_list[k];
					data->image[(i * data->width * blocksize + p * data->width + j * blocksize + q) * 4 + 1] = g_list[k];
					data->image[(i * data->width * blocksize + p * data->width + j * blocksize + q) * 4 + 2] = b_list[k];
				}
			}
		}
		...
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < m; j++)
			{
				for (int k = 0; k < blocksize * blocksize - 1; k += 2)
				{
					...
					//generate the same random numbers but backwards this time
					rt1 = data->sub->get_new_couple(r1, r2, false);
					rt2 = r1 + r2 - rt1;
					
					gt1 = data->sub->get_new_couple(g1, g2, false);
					gt2 = g1 + g2 - gt1;
					
					bt1 = data->sub->get_new_couple(b1, b2, false);
					bt2 = b1 + b2 - bt1;
